...
class BasePlatformTool(BaseTool, ABC):
    ...
    def _connect_browser(self) -> tuple[Any, Any]:
        """
        Connect to existing Chrome via Playwright CDP with stealth injection.
    ...
    def _apply_stealth(self, browser: Any) -> None:
        """Inject stealth scripts into all contexts to bypass automation detection."""
        stealth_js = """
    ...
    def _backup_cookies(self, browser: Any) -> None:
        """Auto-save cookies from all contexts on disconnect for session persistence."""
        ...
        for context in browser.contexts:
            with contextlib.suppress(Exception):
                ...
                if cookies:
                    path = COOKIE_DIR / f"{self.platform}_session.json"
                    path.write_text(
    ...
    def _restore_cookies(self, browser: Any) -> None:
    ...
    def _get_page_text(self, page: Any, selector: str = "body") -> str:
        """Extract text content from page via DOM (no screenshots, token-efficient)."""
        with contextlib.suppress(Exception):
            return page.inner_text(selector, timeout=5000)
        return ""

    def _get_page_snapshot(self, page: Any) -> str:
        """Get accessibility tree snapshot — ultra token-efficient alternative to screenshots."""
        with contextlib.suppress(Exception):
